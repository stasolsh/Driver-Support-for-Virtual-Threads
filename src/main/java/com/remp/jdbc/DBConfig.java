...
public class DBConfig {
    private final static Logger logger = Logger.getLogger("performance.logger");
    ...
    static {
        ...
        try (InputStream input = DBConfig.class.getClassLoader().getResourceAsStream("config.properties")) {
            ...
            logger.info(String.format("Exception during config load: %s with cause: %s", e.getMessage(), e.getCause()));
        }
    }
